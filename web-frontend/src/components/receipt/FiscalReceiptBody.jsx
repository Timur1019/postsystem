...
export default function FiscalReceiptBody({ sale, printAreaId = 'receipt-print-area', className = '' }) {
  ...
  const companyAddress =
    import.meta.env.VITE_COMPANY_ADDRESS || t('fiscalReceipt.defaultAddress');
  const stir = import.meta.env.VITE_COMPANY_STIR || import.meta.env.VITE_STIR || '';
  const fiscalCardId =
  ...
  return (
    ...
    >
      ...
      <footer className="receipt-section mt-4 border-t border-dashed border-black pt-3 text-center">
        <ReceiptQrCode value={qrValue} sizePx={112} />
