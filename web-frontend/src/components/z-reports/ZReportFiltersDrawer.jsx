...
export default function ZReportFiltersDrawer({ open, onClose, filters, onChange, onApply, onReset, stores }) {
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <aside className="relative flex w-full max-w-md flex-col border-l border-slate-200 bg-white shadow-2xl animate-[slideIn_0.2s_ease-out] dark:border-slate-700 dark:bg-slate-900">
        ...
        <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3 dark:border-slate-800">
          <h2 className="font-semibold text-slate-900 dark:text-white">{t('zReports.filterPanelTitle')}</h2>
          <button
            type="button"
            onClick={onClose}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <input type="date" className={inputCls} value={filters.closedFrom} onChange={(e) => set('closedFrom', e.target.value)} />
              </div>
              <div>
                <span className="mb-1 block text-xs text-slate-500">{t('salesLedger.filters.to')}</span>
                <input type="date" className={inputCls} value={filters.closedTo} onChange={(e) => set('closedTo', e.target.value)} />
              </div>
            </div>
          </div>
          <div>
            <label className="mb-1 block text-xs font-medium text-slate-600 dark:text-slate-400">{t('cashRegisters.colFiscal')}</label>
            <input className={inputCls} value={filters.fiscalCardId} onChange={(e) => set('fiscalCardId', e.target.value)} />
          </div>
          <div>
            <label className="mb-1 block text-xs font-medium text-slate-600 dark:text-slate-400">{t('cashRegisters.colSerial')}</label>
            <input className={inputCls} value={filters.terminalSerial} onChange={(e) => set('terminalSerial', e.target.value)} />
          </div>
        </div>
