...
export default function StockProductsFiltersDrawer({ open, onClose, filters, onChange, onApply, onReset }) {
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <aside className="relative flex w-full max-w-md flex-col border-l border-slate-200 bg-white shadow-2xl animate-[slideIn_0.2s_ease-out] dark:border-slate-700 dark:bg-slate-900">
        ...
        <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3 dark:border-slate-800">
          <h2 className="font-semibold text-slate-900 dark:text-white">{t('stockModule.filters.title')}</h2>
          <button
            type="button"
            onClick={onClose}
