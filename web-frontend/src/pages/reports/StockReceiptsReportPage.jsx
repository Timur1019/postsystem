...
export default function StockReceiptsReportPage() {
  ...
  const { data, isPending } = useQuery({
    queryKey: ['stock-receipts-report', params],
    queryFn: () => stockReportApi.receipts(params).then((r) => r.data),
          ...
          <tbody>
            ...
            {rows.map((r) => (
              <tr key={r.id} className="border-t border-slate-100 dark:border-slate-800">
                <td className="px-4 py-2 font-mono text-emerald-800 dark:text-emerald-400">{r.receiptNumber}</td>
                <td className="px-4 py-2">{r.createdAt ? fmtDate(new Date(r.createdAt), 'dd.MM.yyyy HH:mm') : '—'}</td>
                <td className="px-4 py-2">{r.supplierName || '—'}</td>
                <td className="px-4 py-2 text-right">{r.totalQuantity}</td>
                <td className="px-4 py-2 text-right">{fmtMoney(r.totalCost)}</td>
              </tr>
            ))}
          </tbody>
        </table>
