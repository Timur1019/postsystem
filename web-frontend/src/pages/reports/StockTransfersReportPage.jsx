    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <ReportExportButton
            fetchBlob={() => stockReportApi.exportTransfers(exportParams)}
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <select value={fromStoreId} onChange={(e) => { setFromStoreId(e.target.value); setPage(0); }} className="rounded-lg border border-slate-200 px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-900">
          <option value="">{t('stockReports.fromStore')}</option>
          {stores.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
        </select>
        <select value={toStoreId} onChange={(e) => { setToStoreId(e.target.value); setPage(0); }} className="rounded-lg border border-slate-200 px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-900">
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
        <table className="w-full text-sm">
          ...
          <tbody>
            {isPending && <tr><td colSpan={4} className="p-6 text-center">{t('common.loading')}</td></tr>}
            {!isPending && rows.length === 0 && <tr><td colSpan={4} className="p-6 text-center">{t('common.noData')}</td></tr>}
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-slate-100 dark:border-slate-800">
                <td className="px-4 py-3 font-medium">{r.transferNumber}</td>
                <td className="px-4 py-3">{fmtDate(new Date(r.createdAt), 'dd.MM.yyyy HH:mm')}</td>
                <td className="px-4 py-3">{r.fromStoreName} → {r.toStoreName}</td>
