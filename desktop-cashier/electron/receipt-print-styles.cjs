/** Стили чека для скрытого окна печати (без Tailwind, без заливки фона). */
...
#receipt-print-area, .receipt-print-root {
  ...
  max-width: 76mm;
  margin: 0 auto;
  padding: var(--print-pad-y, 2mm) var(--print-pad-x, 3mm);
...
img.receipt-qr, img.receipt-logo {
...
.receipt-secondary { font-size: 11px; font-weight: 600; }
.receipt-meta-label { font-weight: 700; }
.receipt-divider,
.border-t, .border-b, .border-dashed, .border-dotted, .border-black {
...
.col-span-2, .receipt-meta-grid__full { grid-column: 1 / -1; }
